...
int main()
{
	...
	auto a = new Image("src/Test_Files/green-box.png");
	first->addChild(a->getShared());
	game.rootNode.addChild(first->getShared());
	Camera cam1(game.display);
	game.camera = cam1;
	first->addChild(cam1.getShared());
	...
	for (int i = 0; i < 1; i++)
	{
		...
		auto b =
			new Image("src/Test_Files/red-box.png");
		second->addChild(b->getShared());
		game.rootNode.addChild(second->getShared());
		second->setPositionInParent(100, -100);
		obstacles.push_back(second);
	}
	for( CollisionSquareNode* o : obstacles){
		delete o;
	}
	obstacles.clear();
	...
	Process cameraMover = Process([&cam1, &cameraPhysics](double time, ThreadWorker worker)
								  {
	...
	cam1.setPositionInParent(-51, 43);
